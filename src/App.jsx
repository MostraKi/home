import React, { useEffect } from 'react'; import './index.css'; import { initializeApp } from 'firebase/app'; import { getAuth, onAuthStateChanged, GoogleAuthProvider, signInWithPopup, signOut } from 'firebase/auth';
...
return ( <div className="min-h-screen bg-[#fff6ed] text-gray-800 font-sans"> <header className="bg-white h-[60px] flex justify-between items-center px-6 shadow z-10 relative"> <div className="flex items-center gap-2"> <img src="logo.png" alt="Logo" className="h-8" /> <h1 className="text-xl font-bold text-[#008182]"> Mostra<span className="text-[#ec2c4a]">Ki</span> </h1> </div> <div className="flex items-center gap-3"> <span
id="authText"
className="font-bold text-[#15b0af] cursor-pointer hidden"

> </span> <div onClick={() => window.location.href = "/minha-conta"} className="cursor-pointer"> <div  
id="userCircle"  
className="hidden w-8 h-8 rounded-full bg-[#15b0af] text-white flex items-center justify-center text-sm font-bold overflow-hidden"  
/> </div> <div className="text-2xl cursor-pointer md:hidden" onClick={toggleMenu}> ☰ </div> </div>
...
<div  
  ...
  </header>    <div className="relative w-full overflow-hidden">  
  ...
  </div>    <main className="flex flex-col gap-12 px-4 py-12 bg-gray-50 text-center">  
    ...
    </section>  <section className="max-w-xl mx-auto">  
...
<section className="max-w-xl mx-auto">  
  ...
  </main>    <footer className="bg-[#ffe3c6] py-6 text-center text-[#555] text-sm">  
